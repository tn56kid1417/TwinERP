...
const Dashboard = () => {
 ...
 <div>
 <h1 className="text-2xl sm:text-3xl font-medium text-slate-900 tracking-tight">Dashboard Overview</h1>
 <p className="text-slate-500 mt-1 text-sm">Welcome back, {user?.firstName}. Here is what's happening today.</p>
 </div>
 {canEdit && (
 <button 
 onClick={() => setIsQuickActionsOpen(true)}
 className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-colors shadow-sm cursor-pointer"
 >
 ...
 className="bg-white border border-slate-200 rounded-xl shadow-lg w-full max-w-md overflow-hidden"
 >
 <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
 <h3 className="font-semibold text-slate-900 flex items-center gap-2">
 <Zap size={18} className="text-blue-600"/> Quick Actions
 </h3>
 <button 
 ...
 <Link to="/employees"className="flex flex-col items-center justify-center p-6 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 dark:hover:border-slate-300 rounded-xl transition-all group">
 <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-3 shadow-sm transition-transform">
 <UserPlus size={24} className="text-emerald-500"/>
 </div>
 <span className="text-sm font-medium text-slate-700">New Employee</span>
 </Link>
 <Link to="/projects/all"className="flex flex-col items-center justify-center p-6 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 dark:hover:border-slate-300 rounded-xl transition-all group">
 <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-3 shadow-sm transition-transform">
 ...
 {canViewAll && (
 <>
 <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }} className="md:col-span-1 bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between group hover:border-slate-300 transition-all duration-300">
 <div>
 <p className="text-xs font-medium text-slate-500 mb-1 group-hover:text-blue-600 transition-colors">Total Employees</p>
 <h3 className="text-3xl font-medium text-slate-900 origin-left transition-transform">{employees.length}</h3>
 </div>
 ...
 <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }} className="md:col-span-1 bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between group hover:border-slate-300 transition-all duration-300">
 <div>
 <p className="text-xs font-medium text-slate-500 mb-1 group-hover:text-blue-600 transition-colors">Present Today</p>
 <h3 className="text-3xl font-medium text-slate-900 origin-left transition-transform">{presentTodayCount}</h3>
 </div>
 <div className="mt-4 w-full h-1 bg-slate-100 rounded-full overflow-hidden">
 <div className="w-[0%] h-full bg-blue-600 group-hover:bg-blue-500 transition-colors duration-500"style={{ width: employees.length ? `${(presentTodayCount / employees.length) * 100}%` : '0%' }}></div>
 </div>
 </motion.div>
 </>
 )}

 <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 }} className={`bg-white border border-blue-200 p-5 rounded-xl flex flex-col group hover:border-blue-300 transition-all duration-300 relative overflow-hidden ${canViewAll ? 'md:col-span-2' : 'md:col-span-4 max-w-2xl'}`}>
 <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-blue-600/0 via-blue-600/5 to-blue-600/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"/>
 ...
 className="w-full bg-emerald-500 hover:bg-emerald-600 text-white shadow-sm dark:bg-emerald-600 border border-transparent px-4 py-2 rounded-lg text-xs font-bold transition-all h-[38px] flex items-center justify-center gap-2"
 >
 ...
 className="flex-1 bg-amber-500 hover:bg-amber-600 text-white shadow-sm dark:bg-amber-600 border border-transparent px-4 py-2 rounded-lg text-xs font-bold transition-all h-[38px] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 className="flex-1 bg-amber-500 hover:bg-amber-600 text-white shadow-sm dark:bg-amber-600 border border-transparent px-4 py-2 rounded-lg text-xs font-bold transition-all h-[38px] flex items-center justify-center gap-2"
 >
 ...
 disabled={isOnBreak}
 className="flex-1 bg-rose-500 hover:bg-rose-600 text-white shadow-sm dark:bg-rose-600 border border-transparent px-4 py-2 rounded-lg text-xs font-bold transition-all h-[38px] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 <div className="text-[10px] text-slate-500 mt-4 flex flex-col gap-2">
 <div className="flex gap-4">
 <span>Status: <strong className={isClockedOut ? 'text-slate-500' : isOnBreak ? 'text-amber-400' : isClockedIn ? 'text-emerald-400' : 'text-slate-500'}>{isClockedOut ? 'Punched Out' : isOnBreak ? 'On Break' : isClockedIn ? 'Punched In' : 'Not Punched In'}</strong></span>
 {isClockedIn && <span>Time In: <strong className="text-slate-700">{new Date(selectedEmpAttendance!.clockInTime).toLocaleTimeString()}</strong></span>}
 {isClockedOut && <span>Time Out: <strong className="text-slate-700">{new Date(selectedEmpAttendance!.clockOutTime!).toLocaleTimeString()}</strong></span>}
 </div>

 {(isOnBreak || hasTakenBreak || canViewAll) && (
 <div className="flex items-center gap-4 p-2 bg-white/50 rounded-lg border border-slate-200">
 ...
 <strong className="ml-1 text-slate-700">{breakDurationMinutes} mins</strong>
 )}
 </span>
 </div>
 ...
 className="w-16 bg-slate-100 text-slate-900 text-xs px-2 py-1 rounded outline-none focus:ring-1 focus:ring-blue-500 border border-slate-300"
 />
 <button onClick={handleSaveBreakDuration} className="text-emerald-400 hover:text-emerald-300 text-xs font-bold "><CheckCircle size={14} /></button>
 </>
 ) : (
 <button onClick={() => setIsEditingBreak(true)} className="text-slate-500 hover:text-blue-600 flex items-center gap-1 text-[10px] uppercase font-bold tracking-widest transition-colors"><Settings size={12}/> Edit</button>
 )}
 </div>
 )}
 </div>
 ...
 <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 }} className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col group hover:border-slate-300 transition-all duration-300 relative overflow-hidden">
 <h2 className="text-sm font-semibold text-slate-900 mb-4">Department Distribution</h2>
 <div className="h-64">
 ...
 {/* Attendance Today */}
 <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.3 }} className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col group hover:border-slate-300 transition-all duration-300 relative overflow-hidden">
 <h2 className="text-sm font-semibold text-slate-900 mb-4">Attendance Today</h2>
 <div className="h-64">
 <ResponsiveContainer width="100%"height="100%">
 ...
 {canViewAll ? (
 <div className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col group hover:border-slate-300 dark:hover:border-slate-700 shadow-lg shadow-slate-200/40 dark:shadow-black/40 transition-all duration-300">
 <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center group-hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
 <h2 className="text-sm font-semibold text-slate-900 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Recent Employees</h2>
 <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="text-[10px] text-slate-500 hover:text-slate-900 uppercase font-bold tracking-widest transition-colors">View All</motion.button>
 </div>
 <div className="divide-y divide-slate-100 dark:divide-slate-800/60 overflow-y-auto">
 {employees.slice(0, 5).map(emp => (
 <div key={emp.id} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-all cursor-pointer">
 <div className="flex items-center gap-4">
 <div className="w-10 h-10 rounded bg-slate-200 flex items-center justify-center font-semibold text-slate-700 group-hover:bg-slate-300 dark:group-hover:bg-slate-600 transition-colors">
 ...
 <p className="text-[10px] text-slate-500 tracking-tighter">{emp.role}</p>
 </div>
 </div>
 ))}
 {employees.length === 0 && (
 ...
 <div className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col group hover:border-amber-500/40 shadow-lg shadow-slate-200/40 dark:shadow-black/40 transition-all duration-300">
 <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center transition-colors">
 <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2 group-hover:text-amber-400 transition-colors">
 <AwardIcon size={16} className="text-amber-400"/> My Awards
 </h2>
 </div>
 <div className="p-6 space-y-4 overflow-y-auto">
 {myAwards.length === 0 ? (
 <div className="text-center text-slate-500 py-4">No awards yet. Keep up the good work!</div>
 ) : (
 myAwards.map(award => (
 <div key={award.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex gap-4 items-center hover:bg-slate-100 transition-all hover:-translate-y-0.5 cursor-default">
 <div className="w-12 h-12 rounded-full bg-amber-500/10 flex items-center justify-center text-amber-500 flex-shrink-0">
 <AwardIcon size={24} />
 </div>
 <div className="flex-1">
 <h3 className="font-medium text-slate-800">{award.awardType}</h3>
 <p className="text-xs text-amber-400 mt-1 font-semibold">{award.gift}</p>
 {award.description && <p className="text-xs text-slate-500 mt-1">{award.description}</p>}
 </div>
 ...
 <div className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col group hover:border-slate-300 dark:hover:border-slate-700 shadow-lg shadow-slate-200/40 dark:shadow-black/40 transition-all duration-300">
 <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center transition-colors">
 <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
 <Megaphone size={16} className="text-blue-600"/> Announcements
 </h2>
 </div>
 <div className="p-6 space-y-4 overflow-y-auto max-h-[300px]">
 {announcements.length === 0 ? (
 <div className="text-center text-slate-500 py-4">No recent announcements.</div>
 ) : (
 announcements.map(ann => (
 <div key={ann.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 hover:bg-slate-100 transition-all hover:-translate-y-0.5 cursor-default">
 ...
 <div className="mt-3 text-[10px] text-slate-500 uppercase font-bold tracking-widest">
 ...
 <div className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col group hover:border-slate-300 dark:hover:border-slate-700 shadow-lg shadow-slate-200/40 dark:shadow-black/40 transition-all duration-300">
 <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center transition-colors">
 <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2 group-hover:text-blue-600 transition-colors">
 <PartyPopper size={16} className="text-blue-600"/> Upcoming Events
 </h2>
 </div>
 <div className="p-6 space-y-4 overflow-y-auto max-h-[300px]">
 {events.length === 0 ? (
 <div className="text-center text-slate-500 py-4">No upcoming events scheduled.</div>
 ) : (
 events.map(event => (
 <div key={event.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 hover:bg-slate-100 transition-all hover:-translate-y-0.5 cursor-default">
 <h3 className="font-medium text-slate-800 mb-1">{event.title}</h3>
 <div className="text-xs text-slate-600 mb-2 line-clamp-2">{event.description}</div>
 <div className="text-[10px] text-slate-500 font-medium">
 <span className="text-slate-700 mr-2">{new Date(event.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} at {event.time}</span>
 ...
 <div className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col group hover:border-slate-300 dark:hover:border-slate-700 shadow-lg shadow-slate-200/40 dark:shadow-black/40 transition-all duration-300">
 <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center transition-colors">
 <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2 group-hover:text-emerald-400 transition-colors">
 <Calendar size={16} className="text-emerald-400"/> Upcoming Holidays
 </h2>
 </div>
 <div className="p-6 space-y-4 overflow-y-auto max-h-[624px]">
 {holidays.length === 0 ? (
 <div className="text-center text-slate-500 py-4">No upcoming holidays scheduled.</div>
 ) : (
 holidays.map(hol => (
 <div key={hol.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 hover:bg-slate-100 transition-all hover:-translate-y-0.5 cursor-default">
 <div className="flex justify-between items-start mb-2">
 <h3 className="font-medium text-slate-800">{hol.name}</h3>
 <span className="px-2 py-1 bg-slate-100 text-slate-700 border border-slate-300 rounded text-[9px] uppercase font-bold tracking-widest">{hol.type}</span>
 </div>
 <div className="mt-2 text-xs text-slate-600">
 <span className="text-slate-700 font-medium">{hol.startDate}</span>
 {hol.startDate !== hol.endDate && <span> to <span className="text-slate-700 font-medium">{hol.endDate}</span></span>}
 </div>
 <div className="mt-3 text-[10px] text-slate-500 uppercase font-bold tracking-widest">
 ...
 <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center transition-colors">
 <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2 group-hover:text-blue-600 transition-colors">
 <Clock size={16} className="text-blue-600"/> Recent System Activity
 </h2>
 </div>
 <div className="p-6">
 ...
 <div key={activity.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex gap-4 items-start hover:bg-slate-100 transition-all cursor-default">
 ...
 <div className="flex-1">
 <h3 className="font-medium text-slate-800 text-sm mb-1">{activity.title}</h3>
 <p className="text-xs text-slate-600 mb-2">{activity.description}</p>
 <p className="text-[10px] text-slate-500 font-bold ">{activity.date.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
 </div>
 </div>
 ))}
 </div>
