  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto h-full flex flex-col">
      <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-medium text-slate-900 dark:text-white tracking-tight">Leave Balance</h1>
          <p className="text-slate-500 dark:text-slate-500 mt-1 text-sm">View employee leave balances for the current year.</p>
        </div>
        ...
        {canViewAll && employees.length > 0 && (
          <div className="w-full sm:w-64">
            ...
            <select 
              className="w-full bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 rounded px-4 py-2 text-sm focus:outline-none focus:border-indigo-500/50 cursor-pointer"
      ...
      <div className="flex-1 min-h-0 flex">
        <div className="w-full max-w-3xl flex flex-col min-h-0">
          {/* Header Card */}
          <div className="bg-white dark:bg-[#1A1D23] border border-slate-200 dark:border-slate-800 rounded-t-xl p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full border border-slate-300 dark:border-slate-700 bg-slate-800/50 flex items-center justify-center text-indigo-400 shrink-0">
                <User size={24} />
              </div>
              <div>
            ...
            <div className="flex gap-4 sm:gap-8 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-200/80 dark:border-slate-800">
              <div className="text-center border-r border-slate-200 dark:border-slate-800 pr-4 sm:pr-8 flex-1 sm:flex-initial">
                <p className="text-[10px] font-bold text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1">Total</p>
                <p className="text-lg sm:text-xl font-medium text-slate-900 dark:text-white">{totalLeaves}</p>
              </div>
              <div className="text-center border-r border-slate-200 dark:border-slate-800 pr-4 sm:pr-8 flex-1 sm:flex-initial">
