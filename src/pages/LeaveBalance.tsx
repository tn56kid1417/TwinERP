...
const LeaveBalance = () => {
 ...
 <div>
 <h1 className="text-2xl sm:text-3xl font-medium text-slate-900 tracking-tight">Leave Balance</h1>
 <p className="text-slate-500 mt-1 text-sm">View employee leave balances for the current year.</p>
 </div>
 
 {canViewAll && employees.length > 0 && (
 <div className="w-full sm:w-64">
 <label className="block text-[11px] font-medium text-slate-500 mb-1">Select Employee</label>
 <select 
 className="w-full bg-white/50 border border-slate-200 text-slate-700 rounded px-4 py-2 text-sm focus:outline-none focus:border-blue-500/50 cursor-pointer"
 ...
 {/* Header Card */}
 <div className="bg-white border border-slate-200 rounded-t-xl p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
 <div className="flex items-center gap-4">
 <div className="w-12 h-12 rounded-full border border-slate-300 bg-slate-800/50 flex items-center justify-center text-blue-600 shrink-0">
 <User size={24} />
 </div>
 <div>
 ...
 <div className="flex gap-4 sm:gap-8 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-200">
 <div className="text-center border-r border-slate-200 pr-4 sm:pr-8 flex-1 sm:flex-initial">
 <p className="text-[11px] font-medium text-slate-500 mb-1">Total</p>
 <p className="text-lg sm:text-xl font-medium text-slate-900">{totalLeaves}</p>
 </div>
 <div className="text-center border-r border-slate-200 pr-4 sm:pr-8 flex-1 sm:flex-initial">
 <p className="text-[11px] font-medium text-slate-500 mb-1">Used</p>
 <p className="text-lg sm:text-xl font-medium text-red-400">{totalUsed}</p>
 </div>
 <div className="text-center flex-1 sm:flex-initial">
 <p className="text-[11px] font-medium text-slate-500 mb-1">Available</p>
 <p className="text-lg sm:text-xl font-medium text-emerald-400">{totalAvailable}</p>
 </div>
 </div>
 </div>

 {/* List/Table */}
 <div className="bg-white border-x border-b border-slate-200 rounded-b-xl flex-1 flex flex-col min-h-0">
 <div className="px-6 py-4 border-t border-b border-slate-200 bg-white/30/20 flex text-[10px] text-slate-500 font-semibold">
 ...
 <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: idx * 0.05 }} key={leave.id} className="px-6 py-4 border-b border-slate-200/50 flex items-center hover:bg-slate-800/30 transition-colors">
 <div className="flex-1 flex items-center gap-3">
 <div className={`w-2 h-2 rounded-full ${leave.color}`}></div>
 <span className="text-sm font-medium text-slate-700">{leave.name}</span>
 </div>
 <div className="w-24 text-center text-sm text-slate-900 font-medium">{leave.total}</div>
