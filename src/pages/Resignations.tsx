...
const Resignations = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="p-8 max-w-7xl mx-auto h-full flex flex-col">
      <div className="mb-8">
        <h1 className="text-3xl font-medium text-slate-900 dark:text-white tracking-tight">Manage Resignations</h1>
        <p className="text-slate-500 dark:text-slate-500 mt-1">Submit and track employee resignations.</p>
      </div>
        ...
        {showForm && (<div className="lg:col-span-1">
          <div className="bg-white dark:bg-[#1A1D23] rounded-xl border border-slate-200 dark:border-slate-800 p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1">Last Working Date</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1">Reason</label>
                <textarea 
                  required 
                  className="w-full bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 rounded px-4 py-2 text-sm focus:outline-none focus:border-indigo-500/50 h-24 resize-none"
                  ...
                  {loading ? (
                    ...
                    resignations.map((res, idx) => (
                      <motion.tr initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: idx * 0.05 }} key={res.id} className="border-b border-slate-200/80 dark:border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-6 py-4 text-slate-500 dark:text-slate-500 dark:text-slate-400">{res.lastWorkingDate}</td>
                        <td className="px-6 py-4 text-slate-500 dark:text-slate-500 dark:text-slate-400 max-w-xs truncate">{res.reason}</td>
