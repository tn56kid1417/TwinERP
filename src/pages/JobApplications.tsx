...
export default function JobApplications() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'INTERVIEWING':
        return <span className="px-2.5 py-1 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full text-[10px] uppercase font-bold tracking-widest">Interviewing</span>;
      default:
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-3">
          {loading ? (
            ...
            <div className="p-12 text-center bg-white/80 dark:bg-[#0C1017]/80 rounded-2xl border border-slate-200/80 dark:border-slate-800">
              <Users className="h-10 w-10 text-slate-400 mx-auto mb-2 opacity-60" />
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">No applicants in this stage</p>
            ...
            filteredApps.map((app) => (
              <div
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer bg-white/85 dark:bg-[#0C1017]/90 backdrop-blur-xl ${
                  selectedApp?.id === app.id
                    ? 'border-indigo-500 ring-2 ring-indigo-500/20 shadow-md'
                    : 'border-slate-200/80 dark:border-slate-700/50 hover:border-slate-300 dark:hover:border-slate-600 shadow-sm'
                }`}
              >
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/80 text-xs">
                  <div className="text-slate-500 dark:text-slate-400 font-medium">
                    Current Stage: <span className="text-indigo-600 dark:text-indigo-400 font-bold">{getRoundName(app.currentRoundId)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {app.status !== 'HIRED' && app.status !== 'REJECTED' && (
                      <button
                        onClick={(e) => { e.stopPropagation(); handleAdvanceRound(app.id, app.currentRoundId); }}
                        className="px-2.5 py-1 text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 rounded-lg flex items-center gap-1"
                      >
        ...
        <div className="lg:col-span-1">
          {selectedApp ? (
            <div className="sticky top-6 bg-white/90 dark:bg-[#0C1017]/95 backdrop-blur-xl rounded-2xl border border-slate-200/80 dark:border-slate-700/50 p-6 shadow-xl space-y-5">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">{selectedApp.candidateName}</h3>
                  <p className="text-xs text-slate-500">{selectedApp.candidateEmail}</p>
                </div>
                <button onClick={() => setSelectedApp(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-900/60 rounded-xl space-y-2 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-slate-800 dark:text-slate-200">{getRoundName(selectedApp.currentRoundId)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400 font-medium">Applied Date:</span>
                  <span className="text-slate-800 dark:text-slate-200">{new Date(selectedApp.appliedAt).toLocaleDateString()}</span>
                </div>
              </div>

              {selectedApp.notes && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Interview Notes</p>
                  <p className="text-xs text-slate-600 dark:text-slate-300 p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-200/60 dark:border-slate-800">
                    {selectedApp.notes}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                <button
                  ...
                  className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md shadow-indigo-600/20 flex items-center justify-center gap-1.5"
                >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="py-2 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-1.5"
                  >
                    <CheckCircle size={14} /> Hire
                  </button>
                  <button
                    onClick={() => handleStatusChange(selectedApp.id, 'REJECTED')}
                    className="py-2 bg-rose-600/10 hover:bg-rose-600/20 text-rose-600 dark:text-rose-400 border border-rose-500/30 text-xs font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-1.5"
                  >
            ...
            <div className="bg-white/50 dark:bg-slate-900/30 border border-dashed border-slate-300 dark:border-slate-800 rounded-2xl p-8 text-center text-slate-400 text-xs">
