...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0F1115] flex flex-col items-center justify-center p-4">
      ...
      <div className="bg-[#0C1017]/95 backdrop-blur-2xl border border-slate-700/60 rounded-2xl w-full max-w-md p-8 shadow-2xl shadow-black/80 ring-1 ring-white/10 relative overflow-hidden">
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-indigo-500/80 to-transparent" />
        <h2 className="text-xl font-bold text-slate-100 mb-6 tracking-tight">Sign In to Your Account</h2>

        {error && (
          <div className="bg-rose-500/10 border border-rose-500/25 text-rose-400 p-3.5 rounded-xl text-sm mb-6 flex items-center gap-2">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input
              type="email"
              required
              className="w-full bg-[#07090E]/90 border border-slate-700/60 text-slate-100 placeholder:text-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/25 transition-all shadow-inner [color-scheme:dark]"
              value={email}
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/80">
          <p className="text-xs font-semibold text-slate-400 mb-4 text-center">Quick Demo Login</p>
          <div className="grid grid-cols-2 gap-2 max-h-[280px] overflow-y-auto pr-1 custom-scrollbar">
            {DEMO_ACCOUNTS.map(emp => (
              <button
                ...
                disabled={loading}
                className="flex items-center justify-start gap-2.5 bg-[#07090E]/80 hover:bg-[#121722] disabled:opacity-50 border border-slate-800 hover:border-slate-700 text-slate-300 py-2.5 px-3 rounded-xl text-xs font-medium transition-all text-left shadow-sm cursor-pointer"
              >
                <UserCircle size={15} className={`flex-shrink-0 ${emp.role === 'Admin' ? 'text-rose-400' : emp.role === 'CEO' || emp.role === 'CTO' ? 'text-amber-400' : emp.department === 'HR' ? 'text-indigo-400' : 'text-emerald-400'}`} />
                <div className="truncate flex-1">
                  <div className="font-semibold text-slate-100 truncate">{emp.firstName} {emp.lastName}</div>
