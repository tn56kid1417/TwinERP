...
const Holidays = () => {
 ...
 <h1 className="text-2xl sm:text-3xl font-medium text-slate-900 tracking-tight">Manage Holidays</h1>
 <p className="text-slate-500 mt-1 text-sm">Configure company holidays and non-working days.</p>
 </div>
 ...
 <div>
 <label className="block text-xs font-medium text-slate-600 mb-1.5">Holiday Name</label>
 ...
 <div>
 <label className="block text-xs font-medium text-slate-600 mb-1.5">Holiday Type</label>
 <select 
 ...
 >
 <option value="Public Holiday"className="bg-white text-slate-800">Public Holiday</option>
 <option value="Company Holiday"className="bg-white text-slate-800">Company Holiday</option>
 <option value="Optional Holiday"className="bg-white text-slate-800">Optional Holiday</option>
 </select>
 </div>

 <div className="flex items-center gap-2 mt-4">
 <input type="checkbox"id="isPaid"className="accent-blue-600 w-4 h-4 rounded cursor-pointer"
 checked={formData.isPaid} onChange={e => setFormData({...formData, isPaid: e.target.checked})} />
 <label htmlFor="isPaid"className="text-xs text-slate-700 cursor-pointer">Paid Holiday</label>
 </div>
 ...
 <h2 className="text-sm font-semibold text-slate-900">Holidays List</h2>
 </div>
 ...
 <div>
 <p className="text-sm text-slate-700 font-medium">No Holidays found</p>
 <p className="text-xs text-slate-500 mt-1">Get started by creating your first Holiday.</p>
 </div>
 </div>
 </td>
 </tr>
 ...
 <motion.tr initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: idx * 0.05 }} key={hol.id} className="border-b border-slate-200/50 hover:bg-blue-50/40 transition-colors">
 <td className="px-6 py-4 font-medium text-slate-800">{hol.name}</td>
 <td className="px-6 py-4 text-slate-600">{hol.startDate}</td>
 <td className="px-6 py-4 text-slate-600">{hol.endDate}</td>
 <td className="px-6 py-4">
 <span className="px-2 py-1 bg-slate-100 text-slate-700 border border-slate-300 rounded text-[10px] uppercase font-bold tracking-widest">{hol.type}</span>
 </td>
 <td className="px-6 py-4">
 {hol.isPaid ? (
 ...
 <span className="px-2 py-1 bg-slate-100 text-slate-500 border border-slate-300 rounded text-[10px] uppercase font-bold tracking-widest">No</span>
 )}
 </td>
 {canEdit && (
