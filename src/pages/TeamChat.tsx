...
function renderMessageContent(content: string, isOwn: boolean) {
 ...
 : 'bg-blue-100 dark:bg-blue-900/60 text-blue-700 text-blue-600'
 }`}
 >
 {part}
 </span>
...
function MessageBubble({
 ...
 <span className="text-[11px] text-slate-500 italic bg-slate-100/70 px-3 py-1 rounded-full border border-slate-200/40/40">
 {sysText}
 </span>
 </div>
 );
 ...
 {/* Avatar */}
 {!isOwn && (
 <div className={`w-7 h-7 rounded-full bg-${avatarGrad(msg.sender_id)} text-white flex items-center justify-center text-[11px] font-medium shrink-0 shadow-sm`}>
 {initials(senderName)}
 </div>
 )}
 ...
 {!isOwn && (
 <span className="text-[10px] text-slate-400 ml-1 font-medium">{senderName}</span>
 )}

 <div className={`relative rounded-lg px-3.5 py-2.5 text-sm leading-relaxed shadow-sm ${
 isOwn
 ...
 <div className={`mt-2 space-y-2 ${msg.content ? 'pt-1.5 border-t border-white/20' : ''}`}>
 {attachments.map((att) => {
 ...
 : 'bg-slate-100/60 hover:bg-slate-200 text-slate-800'
 }`}
 >
 <div className="p-2 rounded-lg bg-black/10 text-blue-600">
 <FileText size={16} />
 </div>
 <div className="flex-1 min-w-0">
 <p className="font-semibold truncate">{att.file_name}</p>
 <p className="text-[10px] opacity-70 ">{att.mime_type.split('/')[1] || 'FILE'}</p>
 </div>
 ...
 <span className={`text-[9px] ml-1.5 ${isOwn ? 'text-blue-300' : 'text-slate-400'}`}>(edited)</span>
 )}
 </div>
...
export default function TeamChat() {
 ...
 setTimeout(() => el.classList.remove('bg-blue-600/20'), 2500);
 } else {
 ...
 {/* ── Left: Teams List ─────────────────────────────────────────────── */}
 <div className="w-72 shrink-0 flex flex-col bg-white border-r border-slate-200/60/60 overflow-hidden">
 {/* Header */}
 <div className="p-4 border-b border-slate-200/60/60 flex items-center justify-between">
 <div className="flex items-center gap-2">
 <MessageSquare size={18} className="text-blue-600"/>
 <span className="text-sm font-bold text-slate-900">Team Chat</span>
 </div>
 {isElevated && (
 <button
 onClick={() => setShowCreateModal(true)}
 className="p-1.5 rounded-lg text-blue-600 hover:bg-blue-700/10 transition-colors"
 ...
 ? 'bg-blue-600/10 text-blue-600 dark:text-blue-600 border border-blue-600/20'
 : 'text-slate-600 hover:bg-slate-100/80'
 }`}
 >
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-xs font-medium bg-${avatarGrad(team.id)} text-white`}>
 {initials(team.name)}
 </div>
 <div className="flex-1 min-w-0">
 ...
 {team.unreadCount > 0 && (
 <span className="shrink-0 min-w-[20px] h-5 px-1.5 bg-blue-600 text-white text-[11px] font-medium rounded-full flex items-center justify-center">
 ...
 {/* Chat header */}
 <div className="shrink-0 flex items-center justify-between px-5 py-3 border-b border-slate-200/60/60 bg-white ">
 <div className="flex items-center gap-3">
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-medium bg-${selectedTeam ? avatarGrad(selectedTeam.id) : 'from-slate-500 to-slate-400'} text-white`}>
 {selectedTeam ? initials(selectedTeam.name) : '#'}
 </div>
 <div>
 <p className="text-sm font-bold text-slate-900">{selectedTeam?.name}</p>
 ...
 className={`p-2 rounded-lg transition-colors ${searchOpen ? 'text-blue-600 bg-blue-600/10' : 'text-slate-400 hover:text-blue-600 hover:bg-blue-700/10'}`}
 ...
 className="p-2 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-700/10 transition-colors"
 title="Add member"
 >
 <UserPlus size={16} />
 </button>
 ...
 className={`p-2 rounded-lg transition-colors ${showMembers ? 'text-blue-600 bg-blue-600/10' : 'text-slate-400 hover:text-blue-600 hover:bg-blue-700/10'}`}
 ...
 className="bg-slate-100/90/90 border-b border-slate-200/60/60 px-5 py-2.5 flex flex-col gap-2 overflow-hidden"
 >
 <div className="flex items-center gap-2">
 ...
 className="w-full pl-9 pr-4 py-1.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 outline-none focus:border-blue-500"
 />
 </div>
 <button
 onClick={handleSearch}
 disabled={searching || !searchQuery.trim()}
 className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-semibold disabled:opacity-50"
 >
 ...
 className="p-2 bg-white rounded-lg text-xs cursor-pointer hover:bg-blue-700/10 transition-colors flex items-center justify-between gap-2"
 >
 ...
 <button
 onClick={loadMore}
 className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-600 font-semibold bg-blue-600/10 px-3 py-1.5 rounded-md transition-colors"
 >
 ...
 {/* Composer */}
 <div className="shrink-0 p-4 border-t border-slate-200/60/60 bg-white relative">
 {/* Mentions popup autocomplete (Phase 2) */}
 {mentionQuery !== null && mentionCandidates.length > 0 && (
 <div className="absolute bottom-full left-4 mb-2 w-64 bg-white border border-slate-200 rounded-xl shadow-lg p-1 z-30 overflow-hidden">
 ...
 <span className={`text-[10px] ${idx === mentionIndex ? 'text-blue-300' : 'text-slate-400'}`}>
 {cand.role}
 </span>
 </button>
 ))}
 ...
 {/* Edit mode banner */}
 {editingMsg && (
 <div className="flex items-center justify-between bg-blue-600/10 border border-blue-600/20 rounded-xl px-3 py-2 mb-2">
 <span className="text-xs text-blue-600 font-semibold">Editing message</span>
 ...
 <div className="flex flex-wrap gap-2 mb-2 p-2 bg-slate-100 rounded-xl border border-slate-200/60">
 {selectedFiles.map((f, i) => (
 <div key={i} className="flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-lg text-xs text-slate-800 border border-slate-200/60">
 {f.mimeType.startsWith('image/') ? <ImageIcon size={12} className="text-blue-600"/> : <File size={12} className="text-amber-500"/>}
 ...
 <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-100 rounded-xl px-4 py-3">
 ...
 className="p-2.5 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-slate-100 transition-colors"
 ...
 className="flex-1 resize-none bg-slate-100 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 outline-none focus:border-blue-500 dark:focus:border-blue-500 transition-colors max-h-32 overflow-y-auto scrollbar-thin"
 ...
 className="p-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-xl transition-colors flex items-center justify-center shadow-md shadow-blue-600/30"
 >
 {sending
 ...
 className="shrink-0 border-l border-slate-200/60/60 bg-white overflow-hidden"
 >
 ...
 <div className={`w-6 h-6 rounded-full bg-${avatarGrad(m.user_id)} text-white flex items-center justify-center text-[9px] font-bold shrink-0`}>
 ...
 <Hash size={16} className="text-blue-600"/> Create Team Chat
 </h3>
 ...
 className="w-full px-3 py-2 bg-slate-50/80 border border-slate-200 rounded-md text-sm text-slate-900 outline-none focus:border-blue-500"
 />
 </div>

 <div>
 <label className="block text-xs font-bold text-slate-500 mb-1">Initial Members</label>
 <div className="max-h-48 overflow-y-auto space-y-1.5 p-2 bg-slate-50 rounded-xl border border-slate-200/60">
 {allUsers.filter(u => u.id !== user?.id).map(u => (
 <label key={u.id} className="flex items-center gap-2.5 p-1.5 hover:bg-slate-100 rounded-lg cursor-pointer">
 <input
 type="checkbox"
 ...
 <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 text-xs font-bold text-slate-500 hover:bg-slate-100 rounded-md">
 Cancel
 </button>
 <button
 onClick={handleCreateTeam}
 disabled={creating || !newTeamName.trim()}
 className="px-5 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold rounded-md shadow-md shadow-blue-600/30"
 >
 ...
 <div>
 <label className="block text-xs font-bold text-slate-500 mb-1">Select User</label>
 <select
 ...
 <div>
 <label className="block text-xs font-bold text-slate-500 mb-1">Role in Team</label>
 <input
 type="text"
 value={addMemberRole}
 ...
 className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold rounded-md"
 >
 Add Member
 </button>
 </div>
