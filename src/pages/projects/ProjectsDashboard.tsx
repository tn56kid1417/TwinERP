...
export default function ProjectsDashboard() {
  ...
  if (loading) {
    return <div className="p-8 text-slate-500 dark:text-slate-500 dark:text-slate-400">Loading dashboard...</div>;
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto h-full flex flex-col relative overflow-y-auto">
        ...
        <div className="flex items-center gap-4">
          {canViewAll && (
            <button
              ...
              className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-slate-300 dark:border-slate-700"
            >
          ...
          <Link 
            to="/projects/all"
            className="flex items-center gap-2 text-indigo-400 hover:text-indigo-300 text-sm font-medium transition-colors ml-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 text-blue-500 opacity-[0.08] dark:opacity-10 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500">
            <Clock size={40} />
          </div>
          <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Active Projects</p>
          <p className="text-3xl font-semibold text-slate-900 dark:text-white">{activeProjects.length}</p>
        </div>
        
        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 text-amber-500 opacity-[0.08] dark:opacity-10 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500">
            <AlertCircle size={40} />
          </div>
          <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Upcoming Deadlines</p>
          <p className="text-3xl font-semibold text-slate-900 dark:text-white">{upcomingDeadlinesCount}</p>
        </div>

        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 text-indigo-500 opacity-[0.08] dark:opacity-10 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500">
            <ListTodo size={40} />
          </div>
          <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Pending Tasks</p>
          <p className="text-3xl font-semibold text-slate-900 dark:text-white">{pendingTasksCount}</p>
        </div>

        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 text-emerald-500 opacity-[0.08] dark:opacity-10 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500">
            <CheckCircle2 size={40} />
          </div>
          <p className="text-slate-500 dark:text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Completed Projects</p>
          <p className="text-3xl font-semibold text-slate-900 dark:text-white">{completedProjects.length}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl">
          <h2 className="text-lg font-medium text-slate-900 dark:text-white mb-6">Task Status Distribution</h2>
          <div className="h-64">
        ...
        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-medium text-slate-900 dark:text-white">Recent Projects</h2>
            <Link to="/projects/all" className="text-sm text-slate-500 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">See all</Link>
          </div>
          <div className="space-y-4">
            {visibleProjects.slice(0, 4).map(project => (
              <div key={project.id} className="flex items-center justify-between p-4 bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg hover:shadow-sm hover:border-slate-300 dark:hover:shadow-none dark:hover:border-slate-700 transition-all">
                <div>
                  <h3 className="font-medium text-slate-800 dark:text-slate-200 mb-1">{project.name}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-500">{project.client} • Due {new Date(project.deadline).toLocaleDateString()}</p>
                </div>
                <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${
                  ...
                  'bg-slate-500/10 text-slate-500 dark:text-slate-500 dark:text-slate-400'
                }`}>
                  {project.status}
                </span>
              </div>
        ...
        <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl">
          ...
          <div className="space-y-4">
            {activities.slice(0, 5).map(activity => {
              let Icon = Clock;
              let iconColor = 'text-slate-500 dark:text-slate-500 dark:text-slate-400';
              ...
              return (
                <div key={activity.id} className="flex gap-3 p-3 bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg hover:shadow-sm dark:hover:shadow-none hover:border-slate-300 dark:hover:border-slate-700 transition-all">
      ...
      <div className="bg-white dark:bg-[#1A1D23] border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none p-6 rounded-xl mb-8">
        <h2 className="text-lg font-medium text-slate-900 dark:text-white mb-6">Project Timeline</h2>
              ...
              <div className="relative">
                ...
                <div className="absolute top-0 bottom-0 left-[13rem] right-0 pointer-events-none">
                  {gridLines.map((line, i) => (
                    ...
                    >
                      <div className="absolute -top-6 -translate-x-1/2 text-[10px] font-medium text-slate-500 dark:text-slate-500 whitespace-nowrap">
                        {line.label}
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-4 relative mt-8 z-10">
                  {visibleProjects.map(project => {
                    ...
                    if (isCompleted) {
                        ...
                        bgStyle = 'from-blue-500/30 to-blue-600/20 border-blue-500/40 text-blue-100 hover:from-blue-500/40 hover:to-blue-600/30';
                    }
                    else {
                        bgStyle = 'from-indigo-500/30 to-indigo-600/20 border-indigo-500/40 text-indigo-100 hover:from-indigo-500/40 hover:to-indigo-600/30';
                    }

                    return (
                      <div key={project.id} className="flex items-center gap-4 group">
                        <div className="w-48 text-sm font-medium text-slate-700 dark:text-slate-300 truncate group-hover:text-slate-900 dark:text-white transition-colors" title={project.name}>
                          {project.name}
                        </div>
