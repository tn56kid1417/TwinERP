...
const Terminations = () => {
 ...
 <div>
 <h1 className="text-2xl sm:text-3xl font-medium text-slate-900 tracking-tight">Manage Terminations</h1>
 <p className="text-slate-500 mt-1 text-sm">Process and track employee terminations.</p>
 </div>
 <div className="flex items-center gap-2 bg-rose-500/10 text-rose-400 border border-rose-500/20 px-3 py-1.5 rounded-full text-xs font-bold ">
 ...
 <div className="relative bg-white rounded-lg border border-rose-500/20 dark:border-rose-500/30 p-6 shadow-lg shadow-slate-200/50 dark:shadow-black/40 overflow-hidden">
 ...
 <select 
 className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all shadow-sm cursor-pointer"
 ...
 <div>
 <label className="block text-xs font-medium text-slate-600 mb-1.5">Termination Type</label>
 <select 
 className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all shadow-sm cursor-pointer"
 ...
 required
 >
 <option value="Voluntary"className="bg-white text-slate-800">Voluntary</option>
 <option value="Involuntary"className="bg-white text-slate-800">Involuntary</option>
 <option value="Mutual Agreement"className="bg-white text-slate-800">Mutual Agreement</option>
 </select>
 </div>

 <div>
 <label className="block text-xs font-medium text-slate-600 mb-1.5">Notice Date</label>
 <input type="date"required className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all shadow-sm [color-scheme:light] dark:[color-scheme:dark]"
 value={formData.noticeDate} onChange={e => setFormData({...formData, noticeDate: e.target.value})} />
 </div>
 
 <div>
 <label className="block text-xs font-medium text-slate-600 mb-1.5">Termination Date</label>
 <input type="date"required className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all shadow-sm [color-scheme:light] dark:[color-scheme:dark]"
 value={formData.terminationDate} onChange={e => setFormData({...formData, terminationDate: e.target.value})} />
 </div>

 <button type="submit"className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2.5 rounded-xl text-xs transition-all shadow-sm cursor-pointer mt-6">
 Process Termination
 </button>
 </form>
 </div>
 ...
 <h2 className="text-sm font-semibold text-slate-900">Terminations List</h2>
 </div>
 ...
 {loading ? (
 <tr>
 <td colSpan={canEdit ? 6 : 5} className="px-6 py-8 text-center text-slate-500">Loading...</td>
 </tr>
 ) : terminations.length === 0 ? (
 <tr>
 <td colSpan={canEdit ? 6 : 5} className="px-6 py-16 text-center text-slate-500">
 <div className="flex flex-col items-center justify-center space-y-3">
 <Tag size={32} className="text-slate-700"/>
 <div>
 <p className="text-sm text-slate-700 font-medium">No Terminations found</p>
 <p className="text-xs text-slate-500 mt-1">Get started by creating your first Termination.</p>
 </div>
 </div>
 </td>
 </tr>
 ...
 <td className="px-6 py-4 text-slate-600">{res.terminationType}</td>
 <td className="px-6 py-4 text-slate-600">{res.noticeDate}</td>
 <td className="px-6 py-4 text-slate-600">{res.terminationDate}</td>
