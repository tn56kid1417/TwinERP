...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={cn(
        'relative flex flex-col h-screen border-r border-slate-200 dark:border-slate-800 bg-white/60 dark:bg-[#1A1D23]/60 backdrop-blur-md transition-all duration-300 z-30',
        isCollapsed ? 'w-20' : 'w-64'
      )}
    >
      {/* Brand Header */}
      <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 h-16 transition-colors">
        <div className="flex items-center gap-3 overflow-hidden">
          <div className="p-2 bg-primary text-primary-foreground rounded-xl flex items-center justify-center flex-shrink-0">
            <BarChart3 size={20} />
          </div>
          {!isCollapsed && (
            <span className="font-extrabold text-base tracking-tight text-slate-900 dark:text-white dark:text-white bg-gradient-to-r from-primary to-blue-600 bg-clip-text text-transparent truncate">
              Twincord
            </span>
          )}
        </div>
      ...
      <button
        ...
        className="absolute -right-3 top-18 p-1 bg-white text-slate-600 dark:text-slate-300 border border-slate-200 rounded-full hover:bg-slate-50 cursor-pointer shadow-sm z-40 hidden md:block"
      >
            ...
            <Link
              ...
              className={cn(
                ...
                isActive
                  ...
                  : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 hover:text-slate-900 dark:text-white'
              )}
            >
              <Icon size={18} className={cn('flex-shrink-0 transition-transform group-hover:scale-105', isActive ? 'text-current' : 'text-slate-400 group-hover:text-slate-600 dark:text-slate-300')} />
        ...
        <div className={cn('flex items-center gap-3', isCollapsed ? 'justify-center' : 'justify-start')}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-900 dark:text-white dark:text-slate-100 leading-tight truncate">{user.name}</p>
