...
export const Leads: React.FC = () => {
 ...
 >
 {tab.label}
 <Badge variant={activeTab === tab.value ? 'primary' : 'secondary'} className="px-1.5 py-0 text-[11px] font-medium">
 {tab.count}
 </Badge>
 </button>
 ))}
 ...
 <select
 className="w-full text-sm py-2.5 px-3.5 bg-white border border-slate-200 text-slate-800 rounded-md transition-all duration-200 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 cursor-pointer shadow-sm"
 {...register('status')}
 >
 ...
 <option value="Converted"className="bg-white text-slate-800">Converted</option>
 ...
 <div className="flex flex-col gap-1.5 border-t border-slate-100 pt-4 mt-2 text-left">
 <label className="text-xs font-medium text-slate-600 flex items-center gap-1.5">
 <Users size={15} className="text-blue-600 dark:text-blue-600"/> Allocate Assigned Sales Representative
 </label>
 <select
 className="w-full text-sm py-2.5 px-3.5 bg-white border border-slate-200 text-slate-800 rounded-md transition-all duration-200 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 cursor-pointer shadow-sm"
 {...register('assignedUserId')}
 >
 <option value=""className="bg-white text-slate-400">-- Leave Unassigned --</option>
 {salesReps.map((rep) => (
 <option key={rep.id} value={rep.id} className="bg-white text-slate-800">
 {rep.name} ({rep.email})
 </option>
 ))}
 </select>
