      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <Card hoverEffect>
          <CardContent className="p-6 text-left space-y-2">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider block">
              Cumulative Closed Won
            </span>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold text-slate-800 dark:text-slate-100">{formatUSD(totalWonValue)}</span>
              <Badge variant="success" className="font-bold flex items-center gap-0.5">
                <TrendingUp size={10} /> {wonLeads.length} Deals
              </Badge>
            </div>
            <p className="text-xs text-slate-400 dark:text-slate-500">Locked revenue booked to date.</p>
          </CardContent>
        </Card>
        ...
        <Card hoverEffect>
          <CardContent className="p-6 text-left space-y-2">
            ...
            <span className="text-3xl font-extrabold text-slate-800 dark:text-slate-100">{formatUSD(totalOpenValue)}</span>
            <p className="text-xs text-slate-400 dark:text-slate-500">Potential revenue across {openLeads.length} open deals.</p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="flex flex-col items-center justify-center min-h-[300px]">
            {statusData.length === 0 ? (
              <div className="text-slate-400 dark:text-slate-500 text-sm">No active deals found to distribute.</div>
            ) : (
                ...
                <div className="space-y-2 text-left">
                  {statusData.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-sm font-semibold">
                      ...
                      <span className="text-slate-500 dark:text-slate-400 dark:text-slate-500">{item.name}:</span>
                      <span className="text-slate-800 dark:text-slate-100 font-bold">{item.value} deals</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <Card>
          ...
          <CardContent className="min-h-[300px] flex items-center justify-center">
            {repRevenueData.length === 0 ? (
              <div className="text-slate-400 dark:text-slate-500 text-sm flex items-center gap-1">
              ...
              <div className="w-full h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={repRevenueData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" className="text-[11px] fill-slate-500 dark:fill-slate-400 font-medium" />
                    <YAxis className="text-[11px] fill-slate-500 dark:fill-slate-400 font-medium" width={45} tickFormatter={(v: number) => `$${v >= 1000 ? (v / 1000).toFixed(0) + 'k' : v}`} />
                    <Tooltip
        ...
        <CardContent>
          <div className="w-full h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <CartesianGrid strokeDasharray="3 3" className="stroke-slate-200" />
                <XAxis type="number" className="text-[11px] fill-slate-500 dark:fill-slate-400 font-medium" tickFormatter={(v: number) => `$${v >= 1000 ? (v / 1000).toFixed(0) + 'k' : v}`} />
                <YAxis dataKey="stage" type="category" className="text-[11px] fill-slate-500 dark:fill-slate-400 font-medium" width={100} />
                <Tooltip
                  formatter={(val) => formatUSD(Number(val))}
                  contentStyle={{
                    backgroundColor: 'var(--chart-tooltip-bg)',
