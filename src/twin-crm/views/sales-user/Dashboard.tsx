...
export const SalesUserDashboard: React.FC = () => {
 ...
 icon: Clock,
 color: 'text-blue-600 bg-blue-50 border-blue-100 dark:text-blue-600 dark:bg-blue-600/10 dark:border-blue-600/20',
 },
 {
 ...
 <h4 className="font-semibold text-slate-900 leading-snug">{row.name}</h4>
 ...
 {crmModules.map((mod) => (
 <Card key={mod.name} hoverEffect onClick={() => navigate(mod.path)} className="cursor-pointer border-slate-200/60 hover:border-primary/50 transition-colors">
 ...
 <span className="text-xs font-semibold text-slate-500 block ">{stat.title}</span>
 <span className="text-2xl font-extrabold text-slate-900 block mt-0.5 tracking-tight">{stat.value}</span>
 ...
 className="p-3 border border-slate-200 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer text-left space-y-1.5"
 >
 <div className="flex items-center justify-between">
 <h4 className="text-sm font-bold text-slate-900 leading-snug">{lead.name}</h4>
 ...
 <div>
 <h4 className="text-xs font-bold text-slate-900">{activity.text}</h4>
 <p className="text-[11px] text-slate-500 mt-0.5">{activity.detail}</p>
 <span className="text-[9px] text-slate-400 font-bold block mt-1.5">{activity.time}</span>
 </div>
 </div>
 ))}
 </div>
