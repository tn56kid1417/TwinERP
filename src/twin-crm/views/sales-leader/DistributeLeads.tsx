    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <span className="text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">{lead.name}</span>
          <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full', STATUS_COLORS[lead.status])}>
            {lead.status}
          </span>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 mb-3">
        <span>
          Current: <strong className="text-slate-700 dark:text-slate-300">{existingCount}</strong> leads
        </span>
        <span>
  ...
  if (user?.role !== 'SALES_LEADER') {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <p className="text-lg font-semibold text-slate-700 dark:text-slate-300">Access Restricted</p>
        ...
        <Card hoverEffect>
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div className="text-left">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Unassigned</span>
              <span className="text-xl font-extrabold text-slate-900 dark:text-white">{leads.length}</span>
            </div>
          </CardContent>
        </Card>
        <Card hoverEffect>
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div className="text-left">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Team Members</span>
              <span className="text-xl font-extrabold text-slate-900 dark:text-white">{teamMembers.length}</span>
            </div>
          </CardContent>
        </Card>
                  ...
                  {equalPreview.map(({ member, count }) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <Badge variant="primary" className="ml-1 text-[10px] font-bold px-2 py-0.5">
                          +{count}
                        </Badge>
                      </div>
                    </div>
                ...
                <CardContent className="pt-0 space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-4 py-2 text-sm border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
                    />
                    {searchQuery && (
                      <button
                      ...
                      {teamMembers
                        ...
                        .map((m) => (
                          <div key={m.id} className="flex items-center justify-between text-xs">
                            <span className="text-slate-600 dark:text-slate-300 font-medium truncate">{m.name}</span>
