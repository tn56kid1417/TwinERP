...
export const UploadLeads: React.FC = () => {
 ...
 <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-lg shadow-slate-200/50 dark:shadow-black/40 max-w-3xl relative overflow-hidden">
 {message && (
 ...
 <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300/80 border-dashed rounded-md relative bg-slate-50/70/60 hover:bg-slate-100/70 dark:hover:bg-[#07090E]/90 hover:border-blue-600/60 transition-all">
 <div className="space-y-1 text-center">
 <File className="mx-auto h-12 w-12 text-slate-400"/>
 <div className="flex text-sm text-slate-600 justify-center">
 <label htmlFor="file-upload"className="relative cursor-pointer bg-transparent rounded-md font-semibold text-blue-600 dark:text-blue-600 hover:text-blue-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-600">
 ...
 className="w-full px-4 py-2.5 rounded-md border border-slate-200 bg-white text-slate-800 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all shadow-sm cursor-pointer"
 >
 <option value=""className="bg-white text-slate-400">-- Select Leader --</option>
 {leaders.map(l => (
 <option key={l.id} value={l.id} className="bg-white text-slate-800">{l.name} ({l.email})</option>
 ))}
 </select>
 </div>
